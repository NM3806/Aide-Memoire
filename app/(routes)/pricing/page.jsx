...
export default function PricingPage() {
  ...
  const onUpgrade = async () => {
    setLoading(true);
    try {
      // Create a subscription on our server
          ...
          try {
            const verifyResponse = await fetch("/api/razorpay/verify", {
              ...
              body: JSON.stringify({
                ...
                clerkId: user?.id, // Send clerkId to identify the user
              }),
            });
  ...
  // Create a new handler to check for user
  const handleUpgradeClick = () => {
    if (!user) {
      // If no user, redirect to sign-in
      router.push("/sign-in");
    } else {
      // If user exists, proceed with upgrade
      onUpgrade();
    }
  };
